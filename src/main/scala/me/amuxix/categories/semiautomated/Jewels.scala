  ...
  override protected val categoryItems: Future[NonEmptyList[GenericItem]] = Future.successful(
    NonEmptyList.of(
      new GenericItem {
        ...
        override lazy val condition: Future[Condition] = Future.successful(Condition(`class` = "Jewel", rarity = GameRare)) },
      new GenericItem {
        override lazy val rarity: Future[FilterRarity] = Future.successful(Rare)
        override lazy val condition: Future[Condition] = Future.successful(Condition(`class` = "Jewel")) },
    )
  )
