...
class MapFragmentsTable(tag: Tag) extends Table[MapFragment](tag, "map_fragments") with CommonColumns[MapFragment] {
  implicit val currencyTypeMapper = MappedColumnType.base[MapFragmentType, String](
    e => e.toString,
  ...
  private def mapFragmentFactory(name: String, fragmentType: MapFragmentType, dropEnabled: Boolean): MapFragment = fragmentType match {
...
object MapFragments extends BasicOperations[MapFragment, MapFragmentsTable](new MapFragmentsTable(_)) {
  private def getByMapFragmentsType[Type](fragmentType: MapFragmentType): Future[NonEmptyList[Type]] =
    all.map( items =>
      items
        .filter(_.className == fragmentType.toString)
        .sortBy(_.dropLevel)
  ...
  lazy val breachstones: Future[NonEmptyList[Breachstone]] = getByMapFragmentsType(MapFragment.Breachstone)
  lazy val miscsFragments: Future[NonEmptyList[Misc]] = getByMapFragmentsType(MapFragment.Misc)
  lazy val mortalFragments: Future[NonEmptyList[Mortal]] = getByMapFragmentsType(MapFragment.Mortal)
  lazy val prophecyFragments: Future[NonEmptyList[Breachstone]] = getByMapFragmentsType(MapFragment.Breachstone)
  lazy val sacrificeFragments: Future[NonEmptyList[Sacrifice]] = getByMapFragmentsType(MapFragment.Sacrifice)
  lazy val scarabs: Future[NonEmptyList[Scarab]] = getByMapFragmentsType(MapFragment.Scarab)
  lazy val shaperFragments: Future[NonEmptyList[Shaper]] = getByMapFragmentsType(MapFragment.Shaper)
}
