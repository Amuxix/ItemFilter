...
import me.amuxix.actions.IconSize.IconSize
...
trait ImplicitConversions {
  ...
  /*implicit def tuple32OptionBorder(color: (Int, Int, Int)): Option[BorderColor] = {
    (Color.apply(_, _, _)).tupled
    Some((BorderColor.apply _).tupled(color))
  }*/
  ...
  implicit def int2OptionDropLevel(int: Int): Option[DropLevel] = Some(DropLevel(int))
  implicit def tuple22OptionItemLevel(tuple: (Int, Int)): Option[ItemLevel] = Some(ItemLevel(tuple._1, tuple._2))
  implicit def int2OptionItemLevel(int: Int): Option[ItemLevel] = Some(ItemLevel(int))
  implicit def tuple22OptionQuality2(tuple: (Int, Int)): Option[Quality] = Some(Quality(tuple._1, tuple._2))
  implicit def int2OptionQuality(int: Int): Option[Quality] = Some(Quality(int))
  implicit def tuple22OptionRarity(tuple: (GameRarity, GameRarity)): Option[Rarity] = Some(Rarity(tuple._1, tuple._2))
  implicit def r2OptionRarity(r: GameRarity): Option[Rarity] = Some(Rarity(r))
  implicit def tuple22OptionSockets(tuple: (Int, Int)): Option[Sockets] = Some(Sockets(tuple._1, tuple._2))
  implicit def int2OptionSockets(sockets: Int): Option[Sockets] = Some(Sockets(sockets))
  implicit def tuple22OptionLinkedSockets(tuple: (Int, Int)): Option[LinkedSockets] = Some(LinkedSockets(tuple._1, tuple._2))
  ...
  implicit def tuple22OptionHeight(tuple: (Int, Int)): Option[Height] = Some(Height(tuple._1, tuple._2))
  implicit def int2OptionHeight(i: Int): Option[Height] = Some(Height(i))
  implicit def tuple22OptionWidth(tuple: (Int, Int)): Option[Width] = Some(Width(tuple._1, tuple._2))
  ...
  implicit def boolean2OptionSynthesisedItem(boolean: Boolean): Option[SynthesisedItem] = Some(SynthesisedItem(boolean))
  ...
  implicit def tuple22OptionGemLevel(tuple: (Int, Int)): Option[GemLevel] = Some(GemLevel(tuple._1, tuple._2))
  implicit def int2OptionGemLevel(int: Int): Option[GemLevel] = Some(GemLevel(int))
  implicit def tuple22OptionMapTier(tuple: (Int, Int)): Option[MapTier] = Some(MapTier(tuple._1, tuple._2))
